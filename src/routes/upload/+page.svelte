<script>
	...
	$: if (
		$user?.public_id &&
		($user.public_id !== lastUserId || collectionsDirty)
	) {
	...
	async function setCollection(collectionId) {
		try {
			...
			if (newCollection) {
				...
				// Preserve other item properties that might have been set
				if (!item.answer) item.answer = "";
				if (!item.question) item.question = "";
				if (!item.answers) item.answers = [];
				if (!item.src) item.src = "";
				if (!item.supplemental_text) item.supplemental_text = "";
				...
				if (item.numRequired === undefined) item.numRequired = 1;
				...
				// Set items length
				collection.itemsLength = collection.items
				...
				<div class="collection-info row g-3 align-items-center">
							...
							on:uploadImage={async (event) => {
								...
								try {
									...
									if (result) {
										...
										if (
											thumbnailUploader &&
											typeof thumbnailUploader.clearImage ===
												"function"
										) {
					...
					<div class="col-12 col-md">
						...
						>
							<label
								for="privacy-toggle"
								class="form-label me-5 mb-0">Privacy</label
							>
							<input
								id="privacy-toggle"
								type="checkbox"
							...
							<span class="small-text"
								>({collection.itemsLength} questions)</span
							>
						</form>
						<button
							type="button"
		...
		{#if collection}
					...
					{#if collection.questions && collection.questions.length > 0}
						{#each collection.questions as item, index (item)}
							<CollectionItem
								...
								bind:editableItemId
								on:removeItem={async () => {
									const updatedItems = await removeItem(
										item,
										collection.category,
									);
									if (updatedItems) {
										...
										collection.questions =
											updatedItems.questions;
										collection.itemsLength =
											updatedItems.questions.length;
									}
								}}
								on:saveEdit={async (e) => {
								...
								on:updateItem={(e) => {
									const itemIndex =
										collection.questions.findIndex(
											(i) => i.id === e.detail.id,
										);
									if (itemIndex !== -1) {
										const updatedItem = {
			...
			<div class="uploader card mb-4">
				...
				<ul class="nav nav-tabs mb-3">
					<li class="nav-item">
						<a
							class="nav-link {questionType === 'Image'
								? 'active'
								: ''}"
							href="#"
							on:click|preventDefault={() =>
								(questionType = "Image")}>Image</a
						>
					</li>
					<li class="nav-item">
						<a
							class="nav-link {questionType === 'Audio'
								? 'active'
								: ''}"
							href="#"
							on:click|preventDefault={() =>
								(questionType = "Audio")}>Audio</a
						>
					</li>
					<li class="nav-item">
						<a
							class="nav-link {questionType === 'Question'
								? 'active'
								: ''}"
							href="#"
							on:click|preventDefault={() =>
								(questionType = "Question")}>Question</a
						>
					</li>
				</ul>
				{#if questionType === "Image"}
					<form
						class="form row g-2 align-items-center container"
						on:submit|preventDefault
					>
						<div
							...
							<button
								...
								on:click={async () => {
									...
									if (newItems) {
										...
										collection.questions =
											newItems[0].questions;
										collection.itemsLength =
											newItems[0].questions.length;
										addToast({
											type: "success",
										...
										if (
											itemUploader &&
											typeof itemUploader.clearImage ===
												"function"
										) {
						...
						{#if showImageSuggestions}
							<ImageSuggestions
								...
								on:addImage={async (e) => {
									...
									item.category = collection.category;
									console.log(
										"Adding image from suggestions:",
										item,
									);
									const newItem = await uploadData(
										item,
										undefined,
										false,
									);
									console.log(
										"New item from suggestions:",
										newItem,
									);
				...
				{:else if questionType === "Question"}
					<form
						class="form row g-2 align-items-center"
						on:submit|preventDefault
					>
						<div class="col-12">
							<input
			...
			<div class="button-group mt-3 d-flex gap-2">
				{#if collection.itemsLength && collection.itemsLength > 1}
					...
					{:else}
						<button
							class="btn btn-outline-secondary"
							on:click={() => (isReordering = false)}>Done</button
						>
					{/if}
					<button
						class="btn btn-outline-secondary"
				...
				<button
					...
					on:click={() =>
						confirmDelete(collection.id, handleCollectionDeleted)}
