<script>
	...
	// Initialize answer mode based on current item state - only run on component init
	let initialized = false;
	$: if (!initialized && item) {
		if (item.answers && item.answers.length > 0) {
			answerMode = item.isMultipleChoice
				? "multiple-choice"
				: "multi-answer";
		} else {
			answerMode = "single";
	...
	function handleModeChange() {
		console.log("Answer mode changing to:", answerMode);
		switch (answerMode) {
			case "single":
			...
			case "multiple-choice":
				...
				} else if (item.answers.length < 2) {
					item.answers = [...item.answers, ""];
				}
				item.isMultipleChoice = true;
				item.correctAnswerIndex = item.correctAnswerIndex ?? 0;
				item.numRequired = 1;
				item.type = "multiplechoice";
				delete item.answer; // Clear single answer when switching to multiple choice
				break;
			case "multi-answer":
				...
				delete item.isMultipleChoice;
				delete item.correctAnswerIndex;
				if (!item.numRequired || item.numRequired < 1) {
					item.numRequired = Math.min(item.answers.length, 1);
				}

				item.type = "multianswer";
				delete item.answer; // Clear single answer when switching to multi-answer
				break;
		}
		console.log("Item after mode change:", item);
	}
...
<div class="form-group white">
		...
		<div class="multiple-choice-container">
			<div class="mb-2">
				<small class="text-muted"
					>Enter multiple options. Mark the correct answer with a
					checkmark.</small
				>
			</div>
			{#each item.answers as answer, index}
				<div class="input-group mb-2 d-flex align-items-center">
					...
					{#if item.answers.length > 2}
						<button
							type="button"
							class="btn btn-outline-danger d-flex flex-shrink-0"
	...
	{:else if answerMode === "multi-answer"}
		<div class="multi-answer-container">
			<div class="mb-2">
				<small class="text-muted"
					>Enter multiple correct answers. Players need to answer the
					defined number of correct options</small
				>
			</div>
			{#each item.answers as answer, index}
			...
			<div class="multi-answer-controls d-flex gap-2 align-items-center">
				...
				{#if item.answers.length > 1}
					<div class="d-flex align-items-center gap-2">
						<label
							for="numRequired-{idPrefix}"
							class="form-label mb-0">Required:</label
						>
						<input
